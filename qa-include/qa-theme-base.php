...
//	search() outputs <div class="qa-search">, q_list() outputs <div class="qa-q-list">, etc...
    ...
    public function q_view_main($q_view)
    {
        ...
        $this->output('<form method="post">
                        <label>Datasets:
                        <select id="label" name="challengeX" style="margin-top:10px;" onchange="this.form.submit()">');
        ...
        $data = '<form   name="data" method="post" action="./vaqua/data/data.php" target="_blank">
                <input type = "hidden" name = "id"  value = "' . $GLOBALS['qid'] . '">
                <input type="submit" style="padding:5px 15px; 
    background:#ccc; 
    border:0 none;
    cursor:pointer;
    margin-top: 20px; 
    ...
    width: 250px;
    -webkit-border-radius: 5px;
    border-radius: 5px; " value = "Answer this question" cla>
    </form>';
        else {
            $form = '<input type="submit"  style="padding:5px 15px; 
    background:#5DADE2; 
    font-weight: bold;
    border:0 none;
    cursor:pointer;
    ...
    width: 250px;
    -webkit-border-radius: 5px;
    border-radius: 5px; " value="Answer this question" onclick=window.alert("please&nbsp;login&nbsp;or&nbsp;register"); />';
        }
        $this->output($form);
